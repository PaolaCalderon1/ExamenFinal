...
library(sjPlot)
library(sjmisc)
cancer_de_mama <- datos %>% group_by(gen) %>% mutate(row = row_number()) %>%
...
# En la tabla se puede ver genes que se expresan en el cancer de mama 
...
tcga_boxplots <- function(dataset) {
  return(ggplot (dataset, aes(x = gen, y = expresion_level, col = gen)) + labs(x="Gen", y = "Nivel de expresion")
         + geom_boxplot() + geom_jitter(size=0.50))
}
...
tipo_cancer_boxplots = lapply(tipo_cancer, function(x) {ggplot(x, aes(dataset, expresion_level, col = dataset)) +
    geom_boxplot() + theme(legend.position='none') + labs(x = "", y = "Nivel_expresion")})
tipo_cancer_boxplots
...
tipo_cancer_boxplots[3]
ggsave("boxplot3.png", width = 10, height = 12, dpi = 300)
...
data_cancer <- list(BRCA <- filter(df_long, dataset == "BRCA"),
    OV <- filter(df_long, dataset == "OV"),
    LUSC <- filter(df_long, dataset == "LUSC"),
    KIPAN <- filter(df_long, dataset == "KIPAN"),
    KIRP <- filter(df_long, dataset == "KIRP"),
...
grafic_cancer <- list (  BRCA_1 <- BRCA %>% group_by(gen) %>% mutate(row = row_number()) %>%
    tidyr::pivot_wider(names_from = gen, values_from = expresion_level) %>%
    select(-row),
...
reg_gen_expression <- function (gen) {
  if (gen == "GATA3"){
    total <- list (
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GATA3, PTEN)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GATA3, XBP1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GATA3, ESR1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GATA3, MUC1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GATA3, FN1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GATA3, GAPDH)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}))
    }
  else if (gen == "PTEN"){
    total <- list (
      lapply(grafic_cancer, function(x) {ggplot(x, aes(PTEN, GATA3)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(PTEN, XBP1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(PTEN, ESR1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(PTEN, MUC1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(PTEN, FN1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(PTEN, GAPDH)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}))
  }
  else if (gen == "XBP1"){
    total <- list (
      lapply(grafic_cancer, function(x) {ggplot(x, aes(XBP1, GATA3)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(XBP1, PTEN)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(XBP1, ESR1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(XBP1, MUC1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(XBP1, FN1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(XBP1, GAPDH)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}))
  }
  else if (gen == "ESR1"){
    total <- list (
      lapply(grafic_cancer, function(x) {ggplot(x, aes(ESR1, GATA3)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(ESR1, PTEN)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(ESR1, XBP1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(ESR1, MUC1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(ESR1, FN1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(ESR1, GAPDH)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}))
  }
  else if (gen == "MUC1"){
    total <- list (
      lapply(grafic_cancer, function(x) {ggplot(x, aes(MUC1, GATA3)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(MUC1, PTEN)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(MUC1, XBP1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(MUC1, ESR1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(MUC1, FN1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(MUC1, GAPDH)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}))
  }
  else if (gen == "FN1"){
    total <- list (
      lapply(grafic_cancer, function(x) {ggplot(x, aes(FN1, GATA3)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(FN1, PTEN)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(FN1, XBP1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(FN1, ESR1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(FN1, MUC1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(FN1, GAPDH)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}))
  }
  else if (gen == "GAPDH"){
    total <- list (
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GAPDH, GATA3)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GAPDH, PTEN)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GAPDH, XBP1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GAPDH, ESR1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GAPDH, MUC1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}), 
      lapply(grafic_cancer, function(x) {ggplot(x, aes(GAPDH, FN1)) + 
          geom_point() + geom_smooth(method='lm', formula= y~x)}))
  }
}
```
...
cancer <- reg_gen_expression("GATA3")

cancer[[5]][[2]]

ggsave("regresion11.pdf", width = 10, height = 12, dpi = 300)
```
...
# Se puede observar una sobreexpresion del gen LUSC, pues esta sobre la media de los datos
```
